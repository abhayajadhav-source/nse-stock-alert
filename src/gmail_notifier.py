"""
Gmail notification sender.

Uses Python's built-in smtplib + email modules — no extra dependencies.
Sends HTML-formatted emails via Gmail's SMTP server using an App Password.

Two modes (controlled by EMAIL_BATCH_MODE in config):
  - Batch  : one consolidated email per scan cycle (default)
  - Single : one email per stock alert
"""
...
SMTP_PORT = 587   # TLS; use 465 if you switch to SMTP_SSL
...
def _stock_card_html(stock: StockData, news_items: List[NewsItem]) -> str:
    """Render one stock as a styled HTML card."""
...
def _send_email(subject: str, html_body: str, text_body: str) -> bool:
    ...
    if not all([GMAIL_SENDER, GMAIL_APP_PASSWORD, GMAIL_RECIPIENT]):
        logger.error("Gmail credentials missing — set GMAIL_SENDER, "
                     "GMAIL_APP_PASSWORD, GMAIL_RECIPIENT")
        return False
    ...
    # RFC 2046: attach plain text FIRST, then HTML — clients pick the
    # last part they can render, so HTML wins when supported.
...
def send_batch_alert(alerts: List[Tuple[StockData, List[NewsItem]]]) -> bool:
    """
    Send ONE email containing all alerts in the current scan cycle.
    This is the recommended mode — keeps your inbox sane.
    """
    if not alerts:
        return True
...
def send_single_alert(stock: StockData, news_items: List[NewsItem]) -> bool:
    """Send one email per stock — used when EMAIL_BATCH_MODE is False."""
    name = NSE_STOCKS.get(stock.symbol, {}).get("name", stock.symbol)
    tags = ", ".join(get_signal_tags(stock)) or "Tracking"
    ...
    <html><body style="background:#f9fafb;padding:16px;margin:0;
                       font-family:-apple-system,Segoe UI,Roboto,sans-serif;">
      <div style="max-width:680px;margin:0 auto;">
        {_stock_card_html(stock, news_items)}
      </div>
    </body></html>
    """
    text_body = _stock_card_text(stock, news_items)
    subject   = f"{EMAIL_SUBJECT_PREFIX} {stock.symbol} · {tags}"
    return _send_email(subject, html_body, text_body)


def send_test_email() -> bool:
    """Sanity check — confirms credentials work end-to-end."""
    html_body = """
