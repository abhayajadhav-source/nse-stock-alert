...
Uses Google News's public RSS endpoint — no API key, no daily quota,
aggregates Indian financial publications (Moneycontrol, ET, BS, Mint, etc.).
"""
...
def _build_query_url(query: str) -> str:
    """Build Google News RSS URL — India-scoped, last 24h, exact-phrase."""
...
def _check_keywords(title: str) -> List[str]:
    """Return high-priority keywords found in a headline."""
...
def fetch_news_for_stock(symbol: str, max_items: int = 5) -> List[NewsItem]:
    ...
    for entry in feed.entries[:max_items * 2]:
        ...
        if not title:
            continue
        # Sanity check — does the headline actually mention the company?
...
def has_relevant_news(symbol: str) -> tuple[bool, List[NewsItem]]:
    items = fetch_news_for_stock(symbol)
